...
def test_train_pipeline(training_pipeline_params: TrainingPipelineParams, output_model_path_test: str,
                        metric_path_test: str):
    train_pipeline(training_pipeline_params, TRAIN_MODE)
    assert os.path.exists(output_model_path_test)
    ...
    os.remove(output_model_path_test)
